...
export class BreadCrumbModule extends VuexModule {
  ...
  /**
   * Set referral.
   * Fired from router.ts (Router) router.beforeEach hook.
   * @param from 
   */
  @Action
  setFrom(from: string) {
  ...
  setCrumb(crumb: any) {
    /* If crumb allready exists in crumbtrail do not add.
       That means that user has navigated one step back, meaning thar we
       must delete last crumb. */
    let c = this.crumbtrail.find(item => item.path === crumb.path );
