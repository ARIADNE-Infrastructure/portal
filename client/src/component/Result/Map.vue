<template>
  <div v-show="map" class="mb-lg rounded-base border-base border-gray">
    <div class="bg-lightGray items-center p-md flex justify-between">
      ...
      <div class="flex items-center">
        ...
        <help-tooltip title="Search for resources in this area" class="mr-xs" top="2.5rem">
        ...
        <help-tooltip title="Show in fullscreen" top="2.5rem">
          <i class="fas fa-expand transition-color duration-300 hover:text-green px-sm cursor-pointer"
            @click.prevent="navigate('/browse/where?mapq=true')">
          </i>
        </help-tooltip>
      </div>
    </div>
...
export default class ResultMap extends Vue {
  @Prop() title!: string;
  @Prop() height!: string;
