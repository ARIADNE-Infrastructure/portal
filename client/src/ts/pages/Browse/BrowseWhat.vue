<template>
	<div>
    <p v-if="(!wordCloud && !loading) || error" class="text-red">
      {{ error || 'Internal error. Could not load cloud.' }}
    </p>
    <canvas class="max-w-full mx-auto" id="cloud" :class="{ 'cursor-pointer': hovering }" v-on:mouseleave="leaveCanvas"></canvas>
...
export default class BrowseWhat extends Vue {
  ...
  async created () {
    await this.$store.dispatch('setWordCloud');
    this.startWordCloud();
  ...
  get wordCloud () {
    return this.$store.getters.wordCloud();
  }
